...
class Lacak extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $q6 = 'select count(*) jumlah from t_surat_masuk where status = 6';
        $getq6 = $this->db->query($q6)->row();
        $jum6 = $getq6->jumlah;
        $data = array(
            'page' => "tracking",
            'jum1' => $jum1,
    ...
    public function lacakList()
    {
        ...
        foreach ($list as $lacak) {
            ...
            $date_expire = $lacak->tgl_selesai;
            if ($lacak->status == 6) {
                $row[] = "0 hari";
            } elseif ($date_expire == '0000-00-00 00:00:00') {
                $row[] = 'Surat Belum diproses';
            } else {
            ...
            if ($lacak->status == 1) {
                ...
                $row[] = 'Surat diterima dan diproses pelaksana';
            } elseif ($lacak->status == 5) {
                $row[] = 'Laporan Selesai';
            } elseif ($lacak->status == 6) {
    ...
    {
        $this->db = $this->load->database('default', true);
        $qSelesai = "update t_surat_masuk set status = 6, tgl_selesai =now() where id = '$id'";
