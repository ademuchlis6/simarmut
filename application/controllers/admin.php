...
class Admin extends CI_Controller
{
	...
	public function klas_surat()
	{
		...
		if ($mau_ke == "cari") {
			...
			$a['data']		= $this->db->query("SELECT * FROM ref_klasifikasi ORDER BY id ASC LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_klas_surat";
		}
	...
	public function jenis_surat()
	{
		...
		/* pagination */
		$total_row		= $this->db->query("SELECT * FROM ref_jenis")->num_rows();
		$per_page		= 10;
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url() . "index.php/admin/jenis_surat/p");
		...
		if ($mau_ke == "cari") {
			$a['data']		= $this->db->query("SELECT * FROM ref_jenis WHERE nama LIKE '%$cari%' OR uraian LIKE '%$cari%' ORDER BY id DESC")->result();
			$a['page']		= "l_jenis_surat";
		} else if ($mau_ke == "add") {
			$a['page']		= "f_jenis_surat";
		} else if ($mau_ke == "edt") {
			$a['datpil']	= $this->db->query("SELECT * FROM ref_jenis WHERE id = '$idu'")->row();
			$a['page']		= "f_jenis_surat";
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE ref_jenis SET nama = '$nama', waktu = '$waktu', uraian = '$uraian' WHERE id = '$idp'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated</div>");
			redirect('index.php/admin/jenis_surat');
		} else if ($mau_ke == "act_add") {
			$this->db->query("INSERT INTO ref_jenis VALUES('','$kode', '$nama','$waktu', '$uraian')");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been added</div>");
			redirect('index.php/admin/jenis_surat');
		} else if ($mau_ke == "del") {
			$this->db->query("DELETE FROM ref_jenis WHERE id = '$idu'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been deleted</div>");
			redirect('index.php/admin/jenis_surat');
		} else {
			$a['data']		= $this->db->query("SELECT * FROM ref_jenis ORDER BY kode ASC LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_jenis_surat";
		}
	...
	public function surat_masuk()
	{
		...
		$kode					= addslashes($this->input->post('kode'));
		$jenis1 					= addslashes($this->input->post('jenis'));
		$jenis2 = explode("|", $jenis1);
		$jenis = $jenis2[0];
		...
		if ($mau_ke == "del") {
			...
			if ($this->session->userdata('admin_level') == 'Pelaksana') {
				$pelaksana = $this->session->userdata('admin_user');
				$a['data']		= $this->db->query("SELECT * FROM t_surat_masuk WHERE (pelaksana = '$pelaksana' ) and isi_ringkas LIKE '%$cari%' OR indek_berkas LIKE '%$cari%' OR dari LIKE '%$cari%' OR no_surat LIKE '%$cari%' ORDER BY id DESC")->result();
			} else {
			...
			//getArrayKlasifikasi
			$arrKlas = $this->db->query("SELECT id,kode,nama from ref_klasifikasi ")->result_array();
			$a['klas']		= $arrKlas;
			$arrJenisSurat = $this->db->query("SELECT id,nama,waktu from ref_jenis ")->result_array();
			$a['jenisSurat']		= $arrJenisSurat;
		} else if ($mau_ke == "edt") {
			$a['datpil']	= $this->db->query("SELECT * FROM t_surat_masuk WHERE id = '$idu'")->row();
			$jenisA	=  $a['datpil']->jenis;
			$jenisB = $this->db->query("SELECT waktu FROM ref_jenis WHERE id = '$jenisA'")->row();
			// $a['jenisB']		= $jenisB->waktu;
			$arrKlas = $this->db->query("SELECT id,kode,nama from ref_klasifikasi ")->result_array();
			$a['klas']		= $arrKlas;
			$arrJenisSurat = $this->db->query("SELECT id,nama,waktu from ref_jenis ")->result_array();
			$a['jenisSurat']		= $arrJenisSurat;
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$path = $up_data['file_name'];
				// $this->db->query("INSERT INTO t_suxrat_masuk VALUES (NULL, '$kode', '	$no_agenda', '$indek_berkas', '$kegiatan','$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$up_data['file_name']."','$tgl_selesai','$jenis','".$this->session->userdata('admin_id')."')");				
			} else {
				$path = '';
				// $this->db->query("INSERT INTO t_suxrat_masuk VALUES (NULL, '$kode', '$no_agenda', '$indek_berkas','$kegiatan', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '','$tgl_selesai','$jenis','".$this->session->userdata('admin_id')."')");
			}
			$this->db->query(
				"insert into t_surat_masuk
				(kode
				,no_agenda
				,indek_berkas
				,kegiatan
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$path = $up_data['file_name'];
				$wherePath = "file = '$path',";
				//				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', indek_berkas = '$indek_berkas',kegiatan = '$kegiatan', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', file = '".$up_data['file_name']."' WHERE id = '$idp'");
			} else {
				$path = '';
				$wherePath = "";
				//				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', indek_berkas = '$indek_berkas',kegiatan = '$kegiatan', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
			}
			$this->db->query(
				"update t_surat_masuk
				set kode = '$kode'
				,no_agenda = '$no_agenda'
			...
			if ($this->session->userdata('admin_level') == 'Pelaksana') {
				$pelaksana = $this->session->userdata('admin_user');
				$a['data']		= $this->db->query("SELECT * FROM t_surat_masuk WHERE pelaksana = '$pelaksana' and YEAR(tgl_diterima) = '$ta' ORDER BY id DESC LIMIT $awal, $akhir ")->result();
			} else {
	...
	public function surat_keluar()
	{
		...
		if ($mau_ke == "del") {
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda','$perihal', '$uraian', '$dari', '$no_surat','$indeks', '$tgl_surat', NOW(), '$ket', '" . $up_data['file_name'] . "', '" . $this->session->userdata('admin_id') . "','$catatan')");
			} else {
				$this->db->query("INSERT INTO t_surat_keluar VALUES (NULL, '$kode', '$no_agenda','$perihal', '$uraian', '$dari', '$no_surat','$indeks', '$tgl_surat', NOW(), '$ket', '', '" . $this->session->userdata('admin_id') . "','$catatan')");
			}
	...
	public function surat_disposisi()
	{
		...
		$cari					= addslashes($this->input->post('q'));
		$newtgl					= addslashes($this->input->post('newtgl'));
		...
		if ($mau_ke == "del") {
			...
			$arrKasubag = $this->db->query("SELECT nama from t_admin where level = 'Kasubag TU' ")->result_array();
			$a['kasubag']	= $arrKasubag;
			$arrJenisSurat = $this->db->query("SELECT kode,nama,waktu from ref_jenis")->result_array();
			$a['jenisSurat']		= $arrJenisSurat;
			...
			$this->db->query("INSERT INTO t_disposisi (id,id_surat,kpd_yth,isi_disposisi,sifat,jenis,batas_waktu,catatan) VALUES (NULL, '$id_surat', '$kpd_yth', '$isi_disposisi', '$sifat', '$jenis','$batas_waktu', '$catatan')");
			$this->db->query("update t_surat_masuk set status=2 where id = '$id_surat'");
			$this->db->query("update t_surat_masuk set jenis='$jenis' where id = '$id_surat'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been added</div>");
			redirect('index.php/admin/surat_disposisi/' . $id_surat);
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_disposisi SET kpd_yth = '$kpd_yth', isi_disposisi = '$isi_disposisi', sifat = '$sifat',jenis = '$jenis', batas_waktu = '$batas_waktu', catatan = '$catatan' WHERE id = '$idp'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated</div>");
			redirect('index.php/admin/surat_disposisi/' . $id_surat);
		} else {
	...
	public function surat_disposisi_2()
	{
		if ($this->session->userdata('admin_valid') == FALSE && $this->session->userdata('admin_id') == "") {
			redirect("index.php/admin/login");
		}
		...
		if ($this->input->post('kpd_yth')) {
			$a = explode("|", $this->input->post('kpd_yth'));
			$kpd_yth = $a[0];
			$usrPelaksana = $a[1];
		}
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url() . "admin/surat_disposisi_2/" . $idu1 . "/p");

		$a['judul_surat']	= gval("t_surat_masuk", "id", "isi_ringkas", $idu1);

		if ($mau_ke == "del") {
			// $this->db->query("DELETE FROM t_disposisi WHERE id = '$idu2'");
			$this->db->query("update t_disposisi set kpd_yth_2= null, isi_disposisi_2 = null, tgl_dis_2 = null where id = '$idu2'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been deleted </div>");
			redirect('index.php/admin/surat_disposisi_2/' . $idu1);
		} else if ($mau_ke == "add") {
			$arrKasubag = $this->db->query("SELECT nama,username from t_admin where level = 'Pelaksana' ")->result_array();
			$a['kasubag']	= $arrKasubag;
			$a['page']		= "f_surat_disposisi_2";
		} else if ($mau_ke == "edt") {
			$arrKasubag = $this->db->query("SELECT nama,username from t_admin where level = 'Pelaksana' ")->result_array();
			$a['kasubag']	= $arrKasubag;
			$a['datpil']	= $this->db->query("SELECT * FROM t_disposisi WHERE id = '$idu2'")->row();
			...
			$this->db->query("update t_disposisi set kpd_yth_2= '$kpd_yth', isi_disposisi_2 = '$isi_disposisi', tgl_dis_2 = '$batas_waktu' where id_surat = '$id_surat'");
			$this->db->query("update t_surat_masuk set status=3,pelaksana='$usrPelaksana' where id = '$id_surat'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been added</div>");
			redirect('index.php/admin/surat_disposisi_2/' . $id_surat);
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_disposisi SET kpd_yth_2 = '$kpd_yth', isi_disposisi_2 = '$isi_disposisi', tgl_dis_2 = '$batas_waktu' WHERE id = '$idp'");
			$this->db->query("update t_surat_masuk set status=3,pelaksana='$usrPelaksana' where id = '$id_surat'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated</div>");
			redirect('index.php/admin/surat_disposisi_2/' . $id_surat);
		} else if ($mau_ke == "act_edt") {
			$this->db->query("UPDATE t_disposisi SET kpd_yth_2 = '$kpd_yth', isi_disposisi_2 = '$isi_disposisi', tgl_dis_2 = '$batas_waktu' WHERE id = '$idp'");
			$this->db->query("update t_surat_masuk set status=4,pelaksana='$usrPelaksana' where id = '$id_surat'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been updated</div>");
			redirect('index.php/admin/surat_disposisi_2/' . $id_surat);
		} else {
			$a['data']		= $this->db->query("SELECT * FROM t_disposisi WHERE id_surat = '$idu1' LIMIT $awal, $akhir ")->result();
			$a['page']		= "l_surat_disposisi_2";
		}
	...
	{
		$kode 				= $this->input->post('jenis', TRUE);

		$data 				=  $this->db->query("SELECT id, kode, nama,waktu FROM ref_jenis WHERE kode LIKE '%$kode%' ORDER BY id ASC")->result();

		$klasifikasi 		=  array();
		foreach ($data as $d) {
			$json_array				= array();
			$json_array['value']	= $d->waktu;
			$json_array['label']	= $d->kode . " - " . $d->nama;
			$klasifikasi[] 			= $json_array;
		}
	...
	public function disposisi_cetak()
	{
		...
		// $a['datpil2']	= $this->db->query("SELECT kpd_yth,kpd_yth_2 FROM t_disposisi WHERE id_surat = '$idu'")->result();	
		$a['datpil2']	= $this->db->query("SELECT kpd_yth,kpd_yth_2,level FROM t_disposisi a,t_admin b WHERE b.nama=a.kpd_yth and id_surat = '$idu'")->result();
		$a['datpil3']	= $this->db->query("SELECT isi_disposisi,isi_disposisi_2, sifat, batas_waktu, tgl_dis_2 FROM t_disposisi WHERE id_surat = '$idu'")->result();
	...
	{
		$id_surat = $_GET['id'];
		$this->db->query("update t_surat_masuk set status=5 where id = '$id_surat'");
		$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Laporan Selesai</div>");
		redirect('index.php/admin/surat_masuk');
	}
}
