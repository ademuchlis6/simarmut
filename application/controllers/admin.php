...
//				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', indek_berkas = '$indek_berkas',kegiatan = '$kegiatan', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket' WHERE id = '$idp'");
	...
	public function surat_disposisi() {
		...
		if ($mau_ke == "del") {
			...
			$arrKasubag = $this->db->query("SELECT nama from t_admin where level = 'Kasubag' ")->result_array();
			...
			$this->db->query("INSERT INTO t_disposisi (id,id_surat,kpd_yth,isi_disposisi,sifat,batas_waktu,catatan) VALUES (NULL, '$id_surat', '$kpd_yth', '$isi_disposisi', '$sifat', '$batas_waktu', '$catatan')");
	...
	function terima(){
		$id_surat = $_GET['id'];
		$this->db->query("update t_surat_masuk set status=4 where id = '$id_surat'");
